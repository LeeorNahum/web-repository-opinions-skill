...
The case a person sees on a name, filename, path segment, or identifier must be the case that is stored. A case transform applied to a value is a lie the reader cannot detect, and they may type back what they saw, which a case-sensitive system then rejects.
